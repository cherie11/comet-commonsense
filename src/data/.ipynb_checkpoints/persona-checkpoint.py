...
class GenerationDataLoader(DataLoader):
    ...
    def load_data(self, path):
        ...
        for split in self.data:
            file_name = "{}_none_original.txt".format(split)
            with open("{}/{}".format(path, file_name),'r') as f:
                ...
                for l in lines:
                    ...
                    sents.extend((self_sent,other_sent))
            self.data[split]["total"]  = sents
    ...
    def make_tensors(self, text_encoder, special,
                     splits=["train", "valid", "test"], test=False):
        ...
        self.max_effect = max([max([l[1] for l in self.masks[split]["total"]])
                               for split in self.masks])
        print(self.masks['train']["total"][0])
    ...
    def reset_offsets(self, splits=["train", "test", "valid"],
...
def get_generation_sequences(opt, data,  split, text_encoder, test):
    ...
    final_prefix = None
    final_suffix = None
    for prefix in tqdm(data[split]['total']):
